...
def train_model(model, X_tr, X_te, y_tr, y_te):
    ...
    model.compile(optimizer=adam, loss='sparse_categorical_crossentropy',
                  metrics=['accuracy'])
    history = model.fit(X_tr, y_tr, batch_size=256, epochs=100,
...
def train_model_rmsprop(model, X_tr, X_te, y_tr, y_te):
    print(X_tr.shape, X_te.shape, y_tr.shape, y_te.shape)
    model.compile(optimizer=rms_prop, loss='sparse_categorical_crossentropy',
                  metrics=['accuracy'])
    history = model.fit(X_tr, y_tr, batch_size=256, epochs=100,
                        shuffle=True, validation_data=(X_te, y_te),
                        callbacks=[reduce_lr])
...
def ExperimentArchitecture(n_iterations, dataset, logging_file):
    ...
    model_rnn1 = \
        architectures.architecture_rnn_dropout(X_tr, 1, 128, 1, 128)

    model_rnn2 = \
        architectures.architecture_rnn_dropout(X_tr, 1, 16, 1, 128)

    model_rnn3 = \
        architectures.architecture_rnn_dropout(X_tr, 2, 16, 1, 128)

    model_rnn4 = \
        architectures.architecture_rnn_dropout(X_tr, 1, 16, 2, 128)

    model_rnn5 = \
        architectures.architecture_rnn_dropout(X_tr, 2, 16, 2, 128)

    model_rnn6 = \
        architectures.architecture_rnn_dropout(X_tr, 1, 16, 1, 256)

    model_rnn7 = \
        architectures.architecture_rnn_dropout(X_tr, 2, 16, 1, 256)

    model_rnn8 = \
        architectures.architecture_rnn_dropout(X_tr, 1, 16, 2, 256)

    model_rnn9 = \
        architectures.architecture_rnn_dropout(X_tr, 2, 16, 2, 256)

    model_rnn10 = \
        architectures.architecture_rnn_cascade(X_tr, 2, 16, 1, 256)

    model_rnn11 = \
        architectures.architecture_rnn_cascade(X_tr, 3, 16, 1, 256)

    model_rnn12 = \
        architectures.architecture_rnn_cascade(X_tr, 3, 32, 1, 256)

    models_to_train = [model_rnn1, model_rnn2, model_rnn3, model_rnn4, model_rnn5, model_rnn6,
                       model_rnn7, model_rnn8, model_rnn9, model_rnn10, model_rnn11, model_rnn12]
    ...
    for nb, model in enumerate(models_to_train):
        for _iter in range(n_iterations):
            ...
            with open(logging_file, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([model_1.name + '_' + str(_iter),
...
def ExperimentArchitecturesRegularizationActivation(n_iterations, dataset, logging_file):
    if not os.path.isfile(logging_file):
        create_logging_file(logging_file)
    ...
    model_rnn1 = \
        architectures.architecture_rnn_batch_norm_L1_l2(X_tr, 1, 16, 1, 128)

    model_rnn2 = \
        architectures.architecture_rnn_batch_norm_L1_l2(X_tr, 1, 16, 1, 256)

    model_rnn3 = \
        architectures.architecture_rnn_batch_norm_L1_l2(X_tr, 2, 16, 1, 256)

    model_rnn4 = \
        architectures.architecture_rnn_cascade_regularization_bn(X_tr, 2, 16, 1, 256)

    model_rnn5 = \
        architectures.architecture_rnn_cascade_regularization_bn(X_tr, 3, 32, 1, 256)
    ...
    for nb, model in enumerate(models_to_train):
        for _iter in range(n_iterations):
            ...
            with open(logging_file, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([model_1.name + '_' + str(_iter) + 'rms_prop',
                                 report['4']['f1-score'], report['4']['precision'],
                                 report['4']['recall'], report['5']['f1-score'],
                                 report['5']['precision'], report['5']['recall'],
                                 report['6']['f1-score'],
            ...
            with open(logging_file, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([model_1.name + '_' + str(_iter) + 'sdg',
                                 report['4']['f1-score'], report['4']['precision'],
                                 report['4']['recall'], report['5']['f1-score'],
                                 report['5']['precision'], report['5']['recall'],
                                 report['6']['f1-score'],
...
def ExperimentArchitectureRecurrentDropout(n_iterations, dataset, logging_file):
    ...
    # model with batch norm and l1 regularization
    model_rnn = \
        architectures.architecture_rnn_cascade_regularization_bn_dropout(X_tr, 2, 16, 1, 256)

    model_rnn1 = \
        architectures.architecture_rnn_cascade_regularization_bn_dropout(X_tr, 2, 16, 1, 256, dr=0.2)
    ...
    for nb, model in enumerate(models_to_train):
        for _iter in range(n_iterations):
            ...
            model.compile(optimizer=adam, loss='sparse_categorical_crossentropy',
                          metrics=['accuracy'])
            model_hist = model.fit(X_tr, y_tr, batch_size=256, epochs=100,
                                   shuffle=True, validation_data=(X_te, y_te),
            ...
            with open(logging_file, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([model.name + '_' + str(_iter),
                                 report['4']['f1-score'], report['4']['precision'],
                                 report['4']['recall'], report['5']['f1-score'],
                                 report['5']['precision'], report['5']['recall'],
                                 report['6']['f1-score'],
...
def ExperimentArchitectureRecurrentRegularization(n_iterations, dataset, logging_file):
    ...
    # model with batch norm and l1 regularization
    model_rnn = \
        architectures.architecture_rnn_cascade_recurrent_regularization_bn_(X_tr, 2, 16, 1, 256)

    model_rnn1 = \
        architectures.architecture_rnn_cascade_recurrent_regularization_bn_(X_tr, 2, 16, 1, 256, dr=0.2)
