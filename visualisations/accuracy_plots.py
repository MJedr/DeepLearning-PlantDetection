...
def create_accuracy_boxplot(
    data_best_models, column_to_plot_accuracy, model_name_col="model_name"
):
    ...
    plot = sns.boxplot(
        y=data_best_models[column_to_plot_accuracy],
        x=model_name_col,
        data=data_best_models,
        ax=ax,
        color="#d8ede9",
    )
...
def create_accuracy_violin_plot(
    data_best_models, column_to_plot_accuracy, model_name_col="model_name"
):
...
def create_accuracy_dist_plot(
    ...
    for nb, model in enumerate(data_best_models[model_name_col].unique()):
        ...
        plot = sns.kdeplot(
            data_model[column_to_plot_accuracy],
            ax=ax,
            shade=True,
            label="model " + str(nb + 1),
        )
