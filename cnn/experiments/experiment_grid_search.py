...
from cnn.architectures import architecture_regularization1
...
def ExperimentGridSearch(n_iterations, dataset, logging_file, data_type, save=True):
    ...
    for u in range(2):
        ...
        for _iteration in range(n_iterations):
            ...
            model = architecture_regularization1.architecture_basic(
                X_tr,
                ...
                weight_initializer=model_architecture["weight_initializer"],
            )
            ...
            if save:
                ...
                cm.to_csv(
                    f""""..//results//confusion_matrix//cm_{model_name_iteration}_{prefix}.csv"""
                )
                ...
                with open(
                    r"../models/{0}_{1}.json".format(model_name_iteration, data_type),
                    "w",
                ) as json_file:
                    json_file.write(model_json)
                # serialize weights to HDF5
                model_1.save_weights(
                    r"../results/models/{0}_{1}.h5".format(
                        model_name_iteration, data_type
                    )
                )
