...
def classify_raster(
    ...
    with tf.device("/gpu:0"):
        ...
        out_raster_ds = memory_driver.Create(
            out_raster_name, x_im_size, y_im_size, 1, gdal.GDT_UInt16
        )
            ...
            if xx == (xstride - 1):
                if xsize % x_block_size != 0 and ysize % y_block_size != 0:
                    ...
                    array = ds.ReadAsArray(
                        0, int(y_block_size * ystride), xsize, int(ysize % y_block_size)
                    )
                    temp = []
                    for m in range(array.shape[1]):
                        for n in range(array.shape[2]):
                    ...
                    out_raster[ystride * y_block_size : ysize, 0:xsize] = pred
                    ...
                    array = ds.ReadAsArray(
                        0,
                        int(xstride * x_block_size),
                        int(ysize),
                        int(xsize % x_block_size),
                    )
                    ...
                    out_raster[
                        0:ysize,
                        x_block_size * xstride : x_block_size * xstride
                        + xsize % x_block_size,
                    ] = temp
                elif xsize % x_block_size == 0 and ysize % y_block_size != 0:
                    ...
                    for n in range(array.shape[2]):
                        for m in range(array.shape[1]):
                            ...
                            temp[m, n] = p
                    out_raster[
                        y_block_size * ystride : y_block_size * ystride
                        + ysize % y_block_size,
                        0:xsize,
                    ] = temp
                print("classified {0} pixels out of {1}".format(pixels, xsize * ysize))
...
if __name__ == "__main__":
    imagek5 = Config.data_raster
    image = Config.data_raster_time_series
    ...
    paths = [
        model_ts_man,
        model_ts_man_weights,
        model_k5_man,
        model_k5_man_weights,
        model_ts_rs,
        model_ts_rs_weights,
        model_k5_rs,
        model_k5_rs_weights,
    ...
    classify_raster(
        ...
        model_ts_man,
        model_ts_man_weights,
        "classification/ts_manual_search",
        250,
        250,
    )
    ...
    classify_raster(
        ...
        model_k5_man,
        model_k5_man_weights,
        "classification/k5_manual_search",
        250,
        250,
    )
    ...
    classify_raster(
        ...
        model_ts_rs,
        model_ts_rs_weights,
        "classification/ts_random_search",
        250,
        250,
    )
    ...
    classify_raster(
        ...
        model_k5_rs,
        model_k5_rs_weights,
        "classification/k5_random_search",
        250,
        250,
    )
